...
class AuditoriaViewsTest(TestCase):
	def setUp(self):
		User = get_user_model()
		self.user_auditor = User.objects.create_user(username='auditor', password='test123', role='AUDITORIA')
		self.user_normal = User.objects.create_user(username='normal', password='test123', role='SOME')
	...
	def test_lista_auditorias_access(self):
		self.client.login(username='auditor', password='test123')
